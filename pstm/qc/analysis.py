...
def run_geometry_qc(
    ...
    Args:
        ...
        fold_bin_size: Bin size for fold map (default from settings)
    ...
    if fold_bin_size is None:
        fold_bin_size = s.qc.fold_bin_size_m
...
def run_velocity_qc(
    ...
    if min_valid is None:
        min_valid = s.velocity.qc_min_velocity_ms
    if max_valid is None:
        max_valid = s.velocity.qc_max_velocity_ms
...
def verify_diffractor_focus(
    ...
    tolerance_t_ms: float | None = None,
    ...
    Args:
        ...
        tolerance_xy: Spatial tolerance (default from settings)
        tolerance_t_ms: Time tolerance (default from settings)
    ...
    if tolerance_xy is None:
        tolerance_xy = s.qc.focus_tolerance_xy_m
    if tolerance_t_ms is None:
        tolerance_t_ms = s.qc.focus_tolerance_t_ms
...
def verify_flat_reflector_depth(
    ...
    Args:
        ...
        tolerance_ms: Time tolerance (default from settings)
    ...
    if tolerance_ms is None:
        tolerance_ms = s.qc.depth_tolerance_ms
