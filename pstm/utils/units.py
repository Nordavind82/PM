...
def _get_unit_settings():
    """Get unit settings (lazy import to avoid circular dependency)."""
    try:
        from pstm.settings import get_settings
        return get_settings().units
    except ImportError:
        return None


# Conversion constants (defaults, can be overridden via settings)
...
def get_feet_per_meter() -> float:
    """Get feet per meter conversion factor from settings."""
    s = _get_unit_settings()
    return s.feet_per_meter if s else FEET_PER_METER


def get_meters_per_foot() -> float:
    """Get meters per foot conversion factor from settings."""
    s = _get_unit_settings()
    return s.meters_per_foot if s else METERS_PER_FOOT
