...
def example_4_export_data():
    ...
    # Note: n_samples=1001 gives 2000ms recording time at 2ms sample rate
    # This ensures diffractor response (t0~400ms + offset moveout) is captured


    result = create_simple_synthetic(
        diffractor_x=5000.0,      # Diffractor X position (m)
        diffractor_y=5000.0,      # Diffractor Y position (m)
        diffractor_z=800.0,       # Diffractor depth (m)
        survey_extent=10000.0,     # Survey area: 2km x 2km
        grid_spacing=25.0,        # Midpoint spacing: 25m
