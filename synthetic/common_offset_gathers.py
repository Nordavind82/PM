...
def export_to_zarr_parquet(
    ...
    logger.info(f"Exporting to Zarr/Parquet: {output_dir}")
    
    # Export traces to Zarr
    store = LocalStore(str(traces_path))
    z = zarr.create_array(
        store,
        shape=result.traces.shape,
        chunks=(min(1000, result.n_traces), result.n_samples),
        dtype=np.float32,
        overwrite=True,
    )
    z[:] = result.traces
    
    # Save metadata as JSON file alongside zarr
    metadata = {
    ...
    # Export headers to Parquet
    headers_df = pl.DataFrame(result.get_headers_dict())
